...
import { ERROR_OBJECT_KEYS } from './errorFields.js';
...
/** Semantic overrides by field key - only when state_class is missing. Error object per DATA_CONTRACT §5. */
function applySemanticOverrides(field, vizType, gaugeStyle) {
    const key = field.key;
    if (key === 'tsr' || ERROR_OBJECT_KEYS.includes(key)) {
...
export function processFieldConfig(field, deviceSchema, getCategoryFromSchema, getStateClassFromSchema) {
    ...
    if (displayFromMap) {
        ...
        const hasSemanticOverride = ['tsr', ...ERROR_OBJECT_KEYS, 'tv', 'pd'].includes(field.key);
