  ...
  template: `
    @if (!embedded) {
      <header class="page-header">
        <div>
          <h1 class="page-title">Transport settings</h1>
          <p class="page-description">
            Configure device transports. Each transport can run independently.
          </p>
        </div>
    ...
    <!-- Transport tabs -->
    <div class="tabs tabs-boxed mb-6 max-w-2xl">
      <button class="tab" [class.tab-active]="activeTab() === 'lorawan'" (click)="activeTab.set('lorawan')">LoRaWAN</button>
      <button class="tab" [class.tab-active]="activeTab() === 'wifi'" (click)="activeTab.set('wifi')">WiFi</button>
    </div>

    @if (activeTab() === 'lorawan') {
      <!-- LoRaWAN transport config -->
      @if (loading()) {
      ...
      <!-- WiFi transport info -->
      <div class="card-elevated max-w-2xl">
        <div class="card-body-spaced space-y-4">
          <div class="alert alert-info rounded-xl">
            <span>WiFi devices connect via HTTP POST to the ingest endpoint. No broker or gateway hardware needed.</span>
          </div>
...
export class GatewaySettingsComponent implements OnInit {
  ...
  regions = REGIONS;
  activeTab = signal<'lorawan' | 'wifi'>('lorawan');
  loading = signal(true);
