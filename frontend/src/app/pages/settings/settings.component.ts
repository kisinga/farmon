...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, FirmwareCommandsComponent],
  template: `
    ...
    <div class="tabs tabs-boxed bg-base-200/50 p-1 rounded-xl mb-6">
      <button class="tab rounded-lg" [class.tab-active]="activeTab() === 'lorawan'" (click)="activeTab.set('lorawan')">
        LoRaWAN
        @if (!saved()) { <span class="badge badge-warning badge-xs ml-1">!</span> }
      </button>
      <button class="tab rounded-lg" [class.tab-active]="activeTab() === 'wifi'" (click)="activeTab.set('wifi')">
        WiFi
      </button>
    ...
    @switch (activeTab()) {
      ...
      @case ('lorawan') {
        @if (loading()) {
          ...
          @if (!saved()) {
            <div class="alert alert-info rounded-xl mb-6 max-w-2xl">
...
export class SettingsComponent implements OnInit {
  ...
  activeTab = signal<'lorawan' | 'wifi' | 'firmware'>('lorawan');
  loading = signal(true);
