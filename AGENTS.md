...
- **On-device dashboard** (`local.ui`) — the `maji_local_ui` component replaces the stock ESPHome web_server page and serves the operator dashboard from flash: `GET /`, `GET /local/state` (SSE), `POST /local/command`, `POST /local/automations`. The codegen embeds the device-mode app dist (`dist/device/browser`, override `DEVICE_UI_DIST`) into `local-ui-assets.h` as a gzipped PROGMEM asset table — a placeholder page when the dist is absent. Build the app with `npm run build:device -- <site-config.yaml>` (or `DEVICE_UI_CONFIG=…`; the config is required) so the baked topology matches the firmware's site config. App bundle uses hash-location + index fallback for SPA routes. SSE slots are released only via the httpd `free_ctx` destroy callback — never recycle a slot from the stall path.
