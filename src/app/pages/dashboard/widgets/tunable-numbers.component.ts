 ...
 * live value from the shadow and writes the desired value through the
 * command-lifecycle store's single config write path: a PocketBase
 * `controller_config` upsert on the cloud (the server recomputes + republishes the
 * retained /config), one `config_set` command per key over /local/command on-device.
 * Either way convergence shows when the shadow re-publishes the applied number (the
 * field falls back to the live value once the edit clears).
  ...
  template: `
    @if (groups().length) {
      <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-3">
            ...
            @for (sec of g.sections; track sec.label) {
              <div class="flex flex-col gap-1.5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3.5 sm:gap-y-2.5">
                  @for (t of sec.items; track t.key) {
                    <label class="flex flex-col gap-1">
                      <span class="text-xs sm:text-[11px] text-base-content/60 flex items-center gap-1 min-w-0">
                        <span class="truncate">{{ itemLabel(t) }}</span>
                        @if (showUnit(t)) { <span class="text-base-content/30 shrink-0 max-sm:hidden">({{ t.unit }})</span> }
                        @if (t.hint) {
                          <span class="shrink-0 cursor-help text-base-content/30 hover:text-base-content/60 max-sm:hidden" [title]="t.hint">
                      ...
                      @if (t.display === 'toggle') {
                        <div class="flex h-11 sm:h-8 items-center gap-2">
                          <input type="checkbox" class="toggle toggle-md sm:toggle-sm toggle-primary"
                        ...
                        <!-- Phone: 48px field, the unit as a suffix chip inside it, the
                             hint printed underneath (no tooltips on touch). Desktop
                             keeps the dense input-sm + parenthetical + title tooltip. -->
                        <span class="relative block">
                          <input type="number" [min]="t.min" [max]="t.max" [step]="t.step"
                            [class]="'input input-bordered input-md sm:input-sm w-full' + (showUnit(t) ? ' max-sm:pr-14' : '')"
                            [value]="display(g.controller, t)"
                            [disabled]="!canEdit() || !g.online"
                            [placeholder]="t.default"
                            (input)="onInput(g.controller, t, $event)" />
                          @if (showUnit(t)) {
                            <span class="sm:hidden absolute right-3 top-1/2 -translate-y-1/2 rounded-md bg-base-200 px-1.5 py-0.5 text-[11px] text-base-content/50 pointer-events-none">{{ t.unit }}</span>
                          }
                        </span>
                        @if (t.hint) {
                          <span class="sm:hidden text-[11px] leading-snug text-base-content/45">{{ t.hint }}</span>
                        }
                      }
                    </label>
                  }
        ...
        @if (canEdit()) {
          <!-- Phone: the save bar pins to the bottom of the scroll port so it's
               reachable after a long field list; desktop stays an inline row. -->
          <div class="flex max-sm:flex-col items-stretch sm:items-center gap-2 max-sm:sticky max-sm:bottom-0 max-sm:bg-base-100 max-sm:py-3 max-sm:border-t max-sm:border-base-300/30">
            <button class="btn btn-md sm:btn-sm btn-primary sm:w-24" [disabled]="!hasSendableEdit()" (click)="save()">
            ...
            @if (saveError()) { <span class="text-[11px] text-error">{{ saveError() }}</span> }
          </div>
        }
      </div>
    }
...
export class TunableNumbersComponent {
  ...
  /** Last save failure, shown beside the Save bar; the edits are kept. */
  ...
  /** Write every dirty edit as desired config: one patch per online controller
   *  through the backend seam (a `controller_config` upsert on the cloud; one
   *  config_set command per key on-device).
  ...
  protected async save(): Promise<void> {
    ...
    try {
      ...
      // Edits are kept so nothing is lost — surface it and let the operator retry.
