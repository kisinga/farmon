...
export class DeployTabComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    // Auto-populate OTA address from device name
  ...
  async compile() {
    const configName = this.editor.configName();
    ...
    this.error.set(null);
    try {
      await this.electron.esphomeCompile(configName);
    } catch (err) {
      this.error.set(String(err));
      this.running.set(false);
      this.activeAction.set(null);
  ...
  async flash(device?: string) {
    ...
    this.error.set(null);
    try {
      await this.electron.esphomeFlash(configName, device);
    } catch (err) {
      this.error.set(String(err));
      this.running.set(false);
      this.activeAction.set(null);
