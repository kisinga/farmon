  ...
  template: `
    ...
    <mkt-hero size="lg" [blobs]="true" [logo]="true" [waveDivider]="true">
      ...
      <p class="mt-7 mkt-lead text-white/70 max-w-2xl mx-auto">
        MajiFlow meters every litre and runs pumps and valves to the volume you set.
        Stop checking tanks by hand or driving out to start a pump.
      </p>
    ...
    <mkt-section
      ...
      subhead="One system, end to end: the app, the controllers, and off-the-shelf parts a plumber and electrician can install. Design on screen, we set it up, you run it from anywhere.">
    ...
    <mkt-section
      ...
      subhead="From a tap on your phone to water in the field: watch the whole journey play out.">
        ...
        <div class="space-y-5">
          @for (partner of purchasePartners; track partner.name) {
            <article class="overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200 shadow-xl shadow-slate-900/5">
              @if (partner.name === 'Plum') {
                <header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5 sm:px-8">
                  <img src="marketing/plum-logo.png" alt="Plum" width="642" height="338" class="h-12 w-auto" />
                  <div class="flex flex-wrap gap-2">
                    <span class="rounded-full bg-[#205B34] px-3 py-1 text-xs font-semibold text-white">Featured partner</span>
                    <span class="rounded-full bg-[#931C4F] px-3 py-1 text-xs font-semibold text-white">{{ partner.category }}</span>
                  </div>
                </header>

                <div class="p-6 sm:p-8">
                  <p class="max-w-3xl text-sm sm:text-base leading-relaxed text-slate-600">{{ partner.summary }}</p>

                  <div class="mt-4 flex flex-wrap gap-2">
                    @for (item of partner.fit; track item) {
                      <span class="rounded-full bg-[#F5DEB3] px-3 py-1 text-xs font-semibold text-[#072010]">{{ item }}</span>
                    }
                  </div>

                  <div class="mt-5 flex flex-wrap gap-2">
                    <a [href]="partner.website" target="_blank" rel="noopener"
                       class="inline-flex items-center justify-center rounded-full bg-[#931C4F] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#7a173f]">
                      Visit site
                    </a>
                    <a [href]="'tel:' + partner.phone.replaceAll(' ', '')"
                       class="inline-flex items-center justify-center rounded-full bg-[#205B34] px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#174226]">
                      Call
                    </a>
                    <a [href]="'mailto:' + partner.email"
                       class="inline-flex items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold text-[#931C4F] ring-1 ring-[#931C4F] transition-colors hover:bg-[#931C4F]/5">
                      Email
                    </a>
                  </div>

                  <div class="mt-6 border-t border-slate-200 pt-5 text-sm">
                    <p class="font-semibold text-slate-900">Order enquiries</p>
                    <a [href]="'tel:' + partner.phone.replaceAll(' ', '')" class="mt-2 block text-slate-600 hover:text-[#205B34]">{{ partner.phone }}</a>
                    <a [href]="'mailto:' + partner.email" class="mt-1 block text-slate-600 hover:text-[#205B34]">{{ partner.email }}</a>
                  </div>
                </div>
                  ...
                  <div class="mt-4 grid gap-4 md:grid-cols-3">
                    @for (location of partner.locations; track location.label) {
                      <a [href]="location.mapHref" target="_blank" rel="noopener"
                         class="group block rounded-lg border border-slate-200 bg-white p-4 transition-colors hover:border-[#205B34] hover:shadow-sm">
                        ...
                        <span class="mt-3 inline-flex text-xs font-semibold text-[#931C4F]">Open map</span>
                      </a>
                    }
                  </div>
                </div>
                ...
                <div class="border-b border-slate-200 bg-slate-50 px-6 py-5 sm:px-8">
                  ...
                  <h3 class="mt-4 text-3xl font-bold tracking-tight text-slate-950">{{ partner.name }}</h3>
                </div>

                <div class="grid lg:grid-cols-[minmax(0,1fr)_300px]">
                  <div class="p-6 sm:p-8">
                    <p class="max-w-2xl text-sm sm:text-base leading-relaxed text-slate-600">{{ partner.summary }}</p>
          ...
          <div class="max-w-xl mx-auto rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-7">
            ...
            <p class="mt-3 text-sm leading-relaxed text-slate-600">
              We are expanding with irrigation, agrovet, plumbing, and solar partners who already support farms and water sites.
            </p>
            <div class="mt-5 flex flex-col gap-2">
              ...
              <a [href]="whatsappHref" target="_blank" rel="noopener"
                 class="inline-flex w-full items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold text-slate-700 ring-1 ring-slate-300 transition-colors hover:bg-slate-50">
                WhatsApp us
              </a>
            </div>
          </div>
    ...
    <mkt-section [tint]="true"
      ...
      subhead="We assess fit before we quote. MajiFlow is for sites where a dry tank, burnt pump, or missed irrigation window costs real money.">
...
export class LandingComponent {
  ...
  protected readonly ctaButtons: CtaButton[] = [
    { label: 'Request a site assessment', route: '/pricing' },
  ...
  protected readonly capabilities: MktFeatureItem[] = [
    ...
    { title: 'Know before the tank runs dry', body: 'Get warned while there is still water in the tank, so you hear about it before your tenants or guests do.' },
  ];
  ...
  protected readonly purchasePartners: PurchasePartner[] = [
    {
      name: 'Plum',
      category: 'Irrigation, agrovet & pharmacy partner',
      summary:
        'Plum supplies irrigation systems, drip kits, solar pumps and farm inputs, with professional design and installation support.',
      fit: [
