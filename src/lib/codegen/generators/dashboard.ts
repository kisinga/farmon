...
import type { Manifest, ManifestNode } from '@core';
import { nodesByKind, nodesWithFlag, localNodesWithFlag, importedNodesWithFlag, importedNodesByKind } from '@core';
import { NODE_REGISTRY, systemHaEntityIds, networkHaEntityIds, batteryHaEntityIds, automationHaEntityId, routeAutomationAlias, type BoardDef } from '@core';
 ...
 */
function haIds(node: ManifestNode, device: { friendly_name: string }): Partial<Record<import('@core').HaEntityKey, string>> {
 ...
 */
function proxyHaIds(node: Manifest['imports'][number], device: { friendly_name: string }): Partial<Record<import('@core').HaEntityKey, string>> {
