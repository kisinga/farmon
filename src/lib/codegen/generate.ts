...
function generateCompileScript(dir: string): string {
  ...
  return [
    ...
    `    name="\${DEVICE%.yaml}"`,
    `    bin="$(ls -t .esphome/build/"$name"/.pioenvs/"$name"/firmware.ota.bin 2>/dev/null | head -n1 || true)"`,
    `    if [ -z "$bin" ]; then`,
    `      echo "ERROR: could not find a built firmware.ota.bin — did 'compile' succeed?" >&2`,
    `      exit 1`,
    `    fi`,
