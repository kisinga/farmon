...
const flowSensor: NodeDescriptor = {
  ...
  renderSvg: (name) => {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
      ...
      <text x="${cx}" y="${cy + r + 11}" text-anchor="middle" dominant-baseline="middle" font-size="9" font-family="ui-monospace, monospace" font-weight="700" fill="${COLOR}">${escXml(name)}</text>
    </svg>`;
  },
